...
# rishu
    ...
    def counter(self, permission, colorDict, frame, CountArea, mot_tracker, videoName):
        ...
        for i, item in enumerate(track_bbs_ids):
            ...
            c1= (x1+x2)/2
            c2=(y1+y2)/2
            print("\n farme value >>>>>>>>>>>> ", str(id) + "_" + objectName,"   ",mycount,c1,c2)
            df= df.append(pd.DataFrame({"A":[str(id)],"B":[objectName],"C":[mycount ],"D":[c1],"E":[c2]}),ignore_index = True)
            print("DATAFAME>>>>>>>>>>>>>>>>>",df)
